...
//        }
        return postService.updateAdmin(postId, requestDto);
    }
...
//        }
//        AuthenticatedUserInfoDto authenticatedUserInfoDto = jwtUtil.validateAndGetUserInfo(token);
        return postService.update(postId, requestDto, userDetails.getUsername());
    }

    @DeleteMapping("/admin/posts/{postId}")
    @Secured(UserRoleEnum.Authority.ADMIN)
    public ResponseEntity<StatusResponseDto> deletePostingAdmin(@PathVariable Long postId, @AuthenticationPrincipal UserDetails userDetails) {
...
//            throw new IllegalArgumentException("권한이 없습니다.");
//        }
        postService.deletePostingAdmin(postId);
        return new ResponseEntity<>(responseDto, headers, HttpStatus.OK);
    }
    @DeleteMapping("/posts/{postId}")
    public ResponseEntity<StatusResponseDto> deletePosting(@PathVariable Long postId, @AuthenticationPrincipal UserDetails userDetails) {
        StatusResponseDto responseDto = new StatusResponseDto(StatusEnum.OK, "삭제 성공");
        HttpHeaders headers = new HttpHeaders();
        headers.setContentType(new MediaType("application", "json", Charset.forName("UTF-8")));
//        responseDto.setStatus(StatusEnum.OK);
...
//        }
//        AuthenticatedUserInfoDto authenticatedUserInfoDto = jwtUtil.validateAndGetUserInfo(token);
        postService.deletePosting(postId, userDetails.getUsername());
        return new ResponseEntity<>(responseDto, headers, HttpStatus.OK);
    }
}
